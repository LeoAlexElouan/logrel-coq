...
| PairLRPair : forall (A' B' a b : term)
  ...
  (rsnd : forall {Δ} (ρ : Δ ≤ Γ) (h : [ |- Δ ]),
      dSplit (fun Ξ wfΞ ρ' hSplit => [hSplit | Ξ ||- b⟨ρ⟩⟨ρ'⟩ ≅ b⟨ρ⟩⟨ρ'⟩ : (SigRedTyPack.codL ΣA)[a⟨ρ⟩ .: (ρ >> tRel)]⟨ρ'⟩]) (ΣA.(PolyRedPack.posRed) ρ h (rfst ρ h))),
...
Module SigRedTmEq.
  ...
  : Type := {
    ...
    eqSnd [Δ] (ρ : Δ ≤ Γ) (h : [ |- Δ ]) :
      dSplit (fun Ξ wfΞ ρ' hSplit => [hSplit| Ξ ||- (tSnd redL.(nf)⟨ρ⟩)⟨ρ'⟩ ≅ (tSnd redR.(nf)⟨ρ⟩)⟨ρ'⟩ : _]) (ΣA.(PolyRedPack.posRed) ρ h (eqFst ρ h));
  }.
