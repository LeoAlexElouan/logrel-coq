...
Section PolyRed.
  ...
  Lemma beta_pack@{i j k l} {PA : PolyRedPack@{k l} Γ shp shp' pos pos'}
    (PAad : PolyRedPackAdequate@{k l} (LogRel@{i j k l} l) PA)
    : toPack (from PAad) = PA.
  Proof. reflexivity. Qed.

  Lemma beta_ad@{i j k l} {PA : PolyRedPack@{k l} Γ shp shp' pos pos'}
    (PAad : PolyRedPackAdequate@{k l} (LogRel@{i j k l} l) PA)
    : toAd (from PAad) = PAad.
  Proof. reflexivity. Qed.
...
Section ParamRedTy.
  ...
  Definition from@{i j k l} {PA : ParamRedTyPack@{k l} (T:=T) Γ A B}
    (PAad : PolyRedPackAdequate@{k l} (LogRel@{i j k l} l) PA) :
    ParamRedTy@{i j k l}.
  Proof.
  ...
  Lemma beta_pack@{i j k l} {PA : ParamRedTyPack@{k l} (T:=T) Γ A B}
    (PAad : PolyRedPackAdequate@{k l} (LogRel@{i j k l} l) PA)
    : toPack (from PAad) = PA.
  Proof. destruct PA, PAad; reflexivity. Qed.

  Lemma beta_ad@{i j k l} {PA : ParamRedTyPack@{k l} (T:=T) Γ A B}
    (PAad : PolyRedPackAdequate@{k l} (LogRel@{i j k l} l) PA)
    : toAd (from PAad) = PAad.
  Proof. destruct PA, PAad; reflexivity. Qed.
...
Section EvenMoreDefs.
  ...
  Definition LRSig' @{i j k l} {l Γ A B} (ΣA : SigRedTy@{i j k l} Γ l A B)
    : [ LogRel@{i j k l} l | Γ ||- A ≅ B] :=
    LRbuild (LRSig (LogRelRec l) _ (ParamRedTy.toAd ΣA)).
...
Instance SigRedTmWhRed `{GenericTypingProperties} {Γ l A B} (ΣA : [Γ ||-Σ<l> A ≅ B])
  : WhRedTm Γ (outTyL ΣA) (SigRedTm ΣA) := fun t Rt => SigRedTmEq.whred Rt.

#[program]
Instance SigRedTmWhRedRel `{GenericTypingProperties} {Γ l A B} (ΣA : [Γ ||-Σ<l> A ≅ B])
  : WhRedTmRel Γ (outTyL ΣA) (SigRedTmEq ΣA) :=
