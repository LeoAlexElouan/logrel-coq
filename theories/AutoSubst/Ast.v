...
Lemma congr_tAlpha {s0 : nat} {t0 : nat} (H0 : s0 = t0) :
  tAlpha s0 = tAlpha t0.
Proof.
...
Lemma congr_tEllElim {s0 : nat} {s1 : ell} {s2 : term} {s3 : term}
  {s4 : term} {s5 : term} {s6 : term} {t0 : nat} {t1 : ell} {t2 : term}
  {t3 : term} {t4 : term} {t5 : term} {t6 : term} (H0 : s0 = t0)
  (H1 : s1 = t1) (H2 : s2 = t2) (H3 : s3 = t3) (H4 : s4 = t4) (H5 : s5 = t5)
  (H6 : s6 = t6) :
  tEllElim s0 s1 s2 s3 s4 s5 s6 = tEllElim t0 t1 t2 t3 t4 t5 t6.
Proof.
exact (eq_trans
         (eq_trans
            (eq_trans
               (eq_trans
                  (eq_trans
                     (eq_trans
                        (eq_trans eq_refl
                           (ap (fun x => tEllElim x s1 s2 s3 s4 s5 s6) H0))
                        (ap (fun x => tEllElim t0 x s2 s3 s4 s5 s6) H1))
                     (ap (fun x => tEllElim t0 t1 x s3 s4 s5 s6) H2))
                  (ap (fun x => tEllElim t0 t1 t2 x s4 s5 s6) H3))
               (ap (fun x => tEllElim t0 t1 t2 t3 x s5 s6) H4))
            (ap (fun x => tEllElim t0 t1 t2 t3 t4 x s6) H5))
         (ap (fun x => tEllElim t0 t1 t2 t3 t4 t5 x) H6)).
Qed.
...
Fixpoint ren_term (xi_term : nat -> nat) (s : term) {struct s} : term :=
  ...
  | tEllElim s0 s1 s2 s3 s4 s5 s6 =>
      tEllElim s0 s1 (ren_term (upRen_term_term xi_term) s2)
        (ren_term (upRen_term_term xi_term) s3)
        (ren_term (upRen_term_term xi_term) s4) (ren_term xi_term s5)
        (ren_term xi_term s6)
  end.
...
term :=
  ...
  | tEllElim s0 s1 s2 s3 s4 s5 s6 =>
      tEllElim s0 s1 (subst_term (up_term_term sigma_term) s2)
        (subst_term (up_term_term sigma_term) s3)
        (subst_term (up_term_term sigma_term) s4) (subst_term sigma_term s5)
        (subst_term sigma_term s6)
  end.
...
subst_term sigma_term s = s :=
  ...
  | tBoolElim s0 s1 s2 s3 =>
      congr_tBoolElim
        ...
        (idSubst_term sigma_term Eq_term s2)
        (idSubst_term sigma_term Eq_term s3)
  | tAlpha s0 => congr_tAlpha (eq_refl s0)
  | tEmpty => congr_tEmpty
  | tEmptyElim s0 s1 =>
      congr_tEmptyElim
        (idSubst_term (up_term_term sigma_term) (upId_term_term _ Eq_term) s0)
  ...
  | tEllElim s0 s1 s2 s3 s4 s5 s6 =>
      congr_tEllElim (eq_refl s0) (eq_refl s1)
