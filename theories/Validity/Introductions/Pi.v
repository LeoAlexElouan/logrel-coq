...
Section PiValidity.
  ...
  Lemma validΠdom {Γ Γ' F F' G G' l}
    {VΓ : [||-v Γ ≅ Γ']}
    (VΠ : [Γ ||-vS<l> tProd F G ≅ tProd F' G' | VΓ]) :
    [Γ ||-vS<l> F ≅ F' | VΓ].
  Proof.
    constructor; intros ? wfΔ ?? Vσ.
    pose proof (RΠ := SvalidTyExt VΠ wfΔ Vσ).
    rewrite 2!subst_prod in RΠ.
    now unshelve eapply (SinstKripke _ (normRedΠ RΠ).(PolyRed.shpRed)).
  Qed.
  ...
  Proof.
    ...
    pose proof (RΠ := SvalidTyExt VΠ wfΔ Vσ).
    rewrite 2!subst_prod in RΠ.
    generalize (instKripkeSubst wfΔ (normRedΠ RΠ).(PolyRed.posRed) _ hd).
  ...
  Lemma substSΠ {Γ Γ' F F' G G' t u l}
    {VΓ : [||-v Γ ≅ Γ']}
    (VΠ : [Γ ||-vS<l> tProd F G ≅ tProd F' G' | VΓ])
    (Vt : [Γ ||-vS<l> t ≅ u : F | VΓ | validΠdom VΠ]) :
    [_ ||-v<l> G[t..] ≅ G'[u..] | VΓ].
  ...
  Lemma PiValidU {Γ Γ' F F' G G'}
    ...
    (VU : [ Γ ||-vS< one > U | VΓ ])
    (VU' : [ Γ ,, F ||-vS< one > U | VΓF ])
    (VFU : [ Γ ||-vS< one > F ≅ F' : U | VΓ | VU ])
    (VGU : [ Γ ,, F ||-vS< one > G ≅ G' : U | VΓF | VU' ]) :
    [ Γ ||-vS< one > tProd F G ≅ tProd F' G' : U | VΓ | UValid VΓ ].
  Proof.
    ...
    assert [_ ||-vS<one> G ≅ G' : _ | _ | UValid v] by irrValid.
    ...
    1: cbn in *; gtyping.
    enough (h : [ Δ ||-S< zero > (tProd F G)[σ] ≅ (tProd F' G')[σ']]) by exact (cumLR h).
    eapply SvalidTyExt; tea; unshelve eapply PiValid; tea.
